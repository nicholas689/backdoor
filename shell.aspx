<%
' Initialize session timeout to 60 minutes
Session.Timeout = 60 ' Timeout in minutes

' Check if the user is already logged in by checking if the session variable is set
If Session("isLoggedIn") = True Then
    ' User is already logged in, proceed to the original code
    Response.Write("You are already logged in!<br>")
Else
    ' Set the username and password (hardcoded for this example)
    Dim validUsername, validPassword
    validUsername = "1337Toku:)"
    validPassword = "1337Toku:)"

    ' Check if the user has submitted the login form
    If Request.Form("username") <> "" And Request.Form("password") <> "" Then
        ' If credentials match
        If Request.Form("username") = validUsername And Request.Form("password") = validPassword Then
            ' User is authenticated, set session variable to indicate login
            Session("isLoggedIn") = True
            Response.Write("Login successful!<br>")
        Else
            ' Incorrect username or password
            Response.Write("Invalid username or password. Please try again.<br>")
            ShowLoginForm
            Response.End
        End If
    Else
        ' Show login form if no credentials were submitted
        ShowLoginForm
        Response.End
    End If
End If
...
' The rest of your original script follows here
